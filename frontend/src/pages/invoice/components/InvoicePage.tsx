...
import { statesList, gstRates } from "../../../data/invoice/statesList";
...
const InvoicePage: FC<Props> = ({ data, pdfMode, onChange }) => {
  ...
  const [gstTable, setGstTable] = useState<number[]>();
  ...
  useEffect(() => {
    let subTotal = 0;
    let gstTotal = 0;
    var gstTable: number[] = new Array(40);
    ...
    invoice.productLines.forEach((productLine) => {
      ...
      const amount =
        quantityNumber && rateNumber ? quantityNumber * rateNumber : 0;
      const match = productLine.taxrate.match(/(\d+ )%/);
      const gstRate = match && amount > 0 ? parseFloat(match[1]) : 0;
      const gst = (amount * gstRate) / 100;
      gstTable[gstRate] = (gstTable[gstRate] ?? 0) + gst;
      gstTotal += gst;
      subTotal += amount;
    });

    setGstTable(gstTable);
    setGstTotal(gstTotal);
    setSubTotal(subTotal);
  }, [invoice.productLines]);

  useEffect(() => {
    setSameState(invoice["companyState"] === invoice["clientState"]);
  }, [invoice.clientState, invoice.companyState]);
      ...
      <Page className="invoice-wrapper" pdfMode={pdfMode}>
        ...
        <View className="flex" pdfMode={pdfMode}>
          <View className="w-50" pdfMode={pdfMode}>
            ...
            <EditableSelect
              ...
              onChange={(value) => handleChange("companyState", value)}
              pdfMode={pdfMode}
            />
          </View>
          <View className="w-50" pdfMode={pdfMode}>
        ...
        <View className="flex mt-40" pdfMode={pdfMode}>
          <View className="w-55" pdfMode={pdfMode}>
            <EditableInput
              className="bold dark mb-5 label"
            ...
            <EditableSelect
              ...
              onChange={(value) => handleChange("clientState", value)}
              pdfMode={pdfMode}
            />
          </View>
        ...
        <View className="mt-30 bg-dark flex" pdfMode={pdfMode}>
          ...
          <View className="w-17 p-4-8" pdfMode={pdfMode}>
            <EditableInput
              className="white bold right label"
              value={invoice.productLineTaxRateLabel}
              onChange={(value) =>
                handleChange("productLineTaxRateLabel", value)
              }
              pdfMode={pdfMode}
            />
          </View>
        ...
        {invoice.productLines.map((productLine, i) => {
          return (
            // pdfMode && productLine.description === "" ? (
          ...
          <View className="w-50 mt-40" pdfMode={pdfMode}>
            <View className="flex" pdfMode={pdfMode}>
              <View className="w-50 p-5" pdfMode={pdfMode}>
                <EditableInput
                  className="label"
                  value={invoice.taxableValueLabel}
                  onChange={(value) => handleChange("taxableValueLabel", value)}
                  pdfMode={pdfMode}
                />
              </View>
              <View className="w-50 p-5" pdfMode={pdfMode}>
